...
def make_funnel(structure_file, topology_file='',
    ...
    if p0_residue:
        if isinstance(p0_residue, int):
            p0_group = u.select_atoms(f"resid {p0_residue}")
        else:
            p0_group = u.select_atoms(f"resname {p0_residue}")
    else:
        # fallback to default CA selection
        p0_group = u.select_atoms('name CA and point %.3f %.3f %.3f 7' % (
            into_the_protein[0],
            into_the_protein[1],
            into_the_protein[2]), periodic=False)
    p0 = []
    p1 = []
...
def get_protein_ligand_ids(structure_file, topology_file='', ligand_name = 'NX2'):
    """
...
def write_plumed_file(p0, p1, protein_IDs, lig_IDs, extent = 6.0, extent_buffer = 1.5, 
                      l_proj = 5.0, u_proj = 30, beta_cent = 15, 
                      s_cent = 20, deposition_pace = 1000,
    ...
    with open('plumed.dat', 'w') as FILE:
        ...
        FILE.write('lwall: LOWER_WALLS ARG=pp.proj AT=%.1f KAPPA=500.0 EXP=2 EPS=1 # Lower Wall (the starting point of the funnel)\n'% l_proj)
        FILE.write('uwall: UPPER_WALLS ARG=pp.proj AT=%.1f KAPPA=500.0 EXP=2 EPS=1 # Upper Wall (the ending point of the funnel)\n'% u_proj)
        FILE.write('\n')
        FILE.write('##################################\n')
        ...
        FILE.write('spring: CONSTANT VALUES=500.0\n')
        FILE.write('\n')
        FILE.write('##############################\n')
        ...
        FILE.write('        SIGMA=0.5,0.3 HEIGHT=0.25 \n')
        FILE.write('        PACE=%i FILE=HILLS \n'% deposition_pace)
        FILE.write('        GRID_MIN=%.1f,%.1f GRID_MAX=%.1f,%.1f GRID_SPACING=0.05,0.6\n'% \
                                       ((l_proj - 2.5),0.0, # proj min, extent min
                                        (u_proj + 2.5),(extent+extent_buffer+0.2))) # proj max, extent max
        FILE.write('        BIASFACTOR=10.0 TEMP=300\n')
        FILE.write('... METAD\n')
        FILE.write('\n')
        FILE.write('PRINT ARG=* STRIDE=%i FILE=COLVAR FMT=%%8.4f\n'% print_pace)
        
def write_opes_plumed_file(p0, p1, protein_IDs, lig_IDs, extent = 60, extent_buffer = 15, 
                      l_proj = 5.0, u_proj = 30.0, beta_cent = 15, 
                      s_cent = 20, deposition_pace = 1000, barrier = 60,
                      print_pace = 1000, write_ProjectionOnAxis = False):
...
def write_run_py(structure_file,topology_file,run_time,lig_ids,p0_ids,p1_ids,lower_wall,upper_wall,wall_buffer,wall_width,s_cent,beta_cent):
    with open('run.py','w') as FILE:
        ...
        FILE.write("\n")
        FILE.write("def get_ligand_indices(coords, parm, lig_name = 'NX2', include_h = True):\n")
        FILE.write("\n")
        FILE.write("    if coords.endswith('.gro'):\n")
        FILE.write("        u = mda.Universe(coords)\n")
        FILE.write("    elif coords.endswith('.rst') or coords.endswith('.rst7') or coords.endswith('.inpcrd'):\n")
